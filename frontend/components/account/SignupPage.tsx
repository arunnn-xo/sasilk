...
export default function SignupPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setToast({ message: error instanceof Error ? error.message : 'Unable to create account', type: 'error' })
      window.setTimeout(() => setToast(null), 3500)
    } finally {
    ...
    <main className="relative overflow-hidden bg-[#FBF9F6]">
      ...
      <div className="mx-auto grid min-h-[720px] max-w-[1280px] gap-8 px-4 py-12 sm:px-6 md:py-16 lg:grid-cols-[0.92fr_1.08fr] lg:px-8">
        <section className="relative overflow-hidden border border-[#A34336]/15 bg-[#300D14] p-8 text-[#FAF6EE] shadow-[0_18px_42px_rgba(48,13,20,0.14)] md:p-10">
          <div className="pointer-events-none absolute inset-3 border border-[#BF9A4B]/25" />
          <div className="pointer-events-none absolute -bottom-24 -right-24 h-72 w-72 rounded-full border border-[#BF9A4B]/20" />
          ...
          <div className="relative z-10 flex min-h-full flex-col justify-between gap-12">
            <div>
              <p className="mb-4 text-xs font-semibold uppercase tracking-[0.3em] text-[#BF9A4B]">SOIL GODDESS Account</p>
            ...
            <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
              {trustItems.map(({ Icon, label }) => (
                <div key={label} className="border border-[#BF9A4B]/25 bg-burgundy/[0.04] p-4">
                  <Icon className="mb-3 h-5 w-5 text-[#BF9A4B]" />
          ...
          <div className="relative z-10">
            ...
            <form className="space-y-5" onSubmit={handleSubmit} noValidate>
              <div className="grid gap-5 sm:grid-cols-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-[#D9B86E] bg-[#FBF9F6] px-4 py-3 text-sm outline-none transition focus:border-[#A34336] focus:bg-white"
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full border border-[#D9B86E] bg-[#FBF9F6] py-3 pl-11 pr-4 text-sm outline-none transition focus:border-[#A34336] focus:bg-white"
              ...
              <button
                ...
                className="group relative flex w-full items-center justify-center gap-2 overflow-hidden bg-[#A34336] py-4 text-sm font-semibold uppercase tracking-[0.2em] text-gold shadow-md transition disabled:cursor-not-allowed disabled:opacity-70"
              >
            ...
            <div className="mt-7 flex flex-col items-center justify-between gap-3 border-t border-gold pt-5 text-sm text-[#666666] sm:flex-row">
      ...
      <div className={`fixed bottom-5 left-1/2 z-[120] flex -translate-x-1/2 items-center gap-2 rounded px-6 py-3 text-gold shadow-xl transition-opacity duration-300 ${toast ? 'opacity-100' : 'pointer-events-none opacity-0'} ${toast?.type === 'error' ? 'bg-[#A34336]' : 'bg-burgundy'}`}>
