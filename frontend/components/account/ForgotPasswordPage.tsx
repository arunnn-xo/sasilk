        ...
        <div className="mx-auto grid min-h-[640px] max-w-[1280px] gap-8 px-4 py-12 sm:px-6 md:py-16 lg:grid-cols-[0.92fr_1.08fr] lg:px-8">
          <section className="relative overflow-hidden border border-[#A34336]/15 bg-[#300D14] p-8 text-[#FAF6EE] shadow-[0_18px_42px_rgba(48,13,20,0.14)] md:p-10">
            ...
            <div className="relative z-10 flex min-h-full flex-col justify-between gap-12">
              <div>
                <p className="font-montserrat mb-4 text-[11px] md:text-xs font-bold uppercase tracking-[0.25em] text-[#C4A462]">Password Reset</p>
    ...
    <main className="relative overflow-hidden bg-[#FBF9F6]">
      ...
      <div className="mx-auto grid min-h-[720px] max-w-[1280px] gap-8 px-4 py-12 sm:px-6 md:py-16 lg:grid-cols-[0.92fr_1.08fr] lg:px-8">
        <section className="relative overflow-hidden border border-[#A34336]/15 bg-[#300D14] p-8 text-[#FAF6EE] shadow-[0_18px_42px_rgba(48,13,20,0.14)] md:p-10">
          ...
          <div className="relative z-10 flex min-h-full flex-col justify-between gap-12">
            <div>
              <Link href="/login" className="mb-8 inline-flex items-center gap-2 text-sm font-medium text-[#C4A462] hover:underline">
              ...
              <p className="font-sans max-w-xl text-sm sm:text-base font-medium leading-relaxed text-[#E8DFD0]">
                {step === 'email'
                  ? "No worries. Enter the email linked to your Soil Goddess account and we'll send you a reset OTP."
                  : step === 'otp'
              ...
              <form className="space-y-5" onSubmit={e => { e.preventDefault(); setError(''); if (otp.trim().length !== 6) { setError('Enter the 6-digit OTP sent to your email.'); return } setStep('password') }} noValidate>
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-[#E8DCC4] bg-[#FBF9F6] px-4 py-3 text-center text-lg font-bold tracking-[0.3em] outline-none transition focus:border-[#A34336] focus:bg-white"
                    placeholder="000000"
                  />
                </div>
            ...
            {step === 'password' && (
              <form className="space-y-5" onSubmit={handleResetPassword} noValidate>
                ...
                <div className="flex gap-3">
                  ...
                  <button type="button" onClick={() => { setStep('otp'); setPassword(''); setConfirmPassword(''); setError('') }} className="px-5 py-4 text-sm font-semibold text-[#A34336] border border-[#E8DCC4] transition hover:bg-[#FBF9F6]">
                    Back
                  </button>
                </div>
                {error ? <p className="text-center text-sm font-medium text-[#A34336]">{error}</p> : null}
