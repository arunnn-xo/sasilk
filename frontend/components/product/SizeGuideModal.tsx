    ...
    >
      ...
      >
        <div className="flex items-center justify-between border-b px-5 py-4">
          <h2 className="text-lg font-semibold text-[#2A1A1E]">
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b bg-gray-50">
                <th className="px-3 py-2.5 font-semibold text-[#2A1A1E]">Size</th>
                {measureKeys.map(key => (
                  <th key={key} className="px-3 py-2.5 font-semibold text-[#2A1A1E]">{key}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {entries.map(entry => (
                <tr key={entry.label} className="border-b last:border-0 hover:bg-gray-50/50">
                  <td className="px-3 py-2.5 font-medium text-[#6B1A2A]">{entry.label}</td>
                  {measureKeys.map(key => (
