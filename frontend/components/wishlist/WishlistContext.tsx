...
export function WishlistProvider({ children }: { children: ReactNode }) {
  ...
  const isWished = useCallback((id: number, variantId?: number | null) => {
    if (variantId !== undefined) {
      return serverItems.some(i => matchesExact(i, id, variantId))
    }
    return serverItems.some(i => matchesProduct(i, id))
  }, [serverItems])
  ...
  const removeAllForProduct = useCallback((id: number, productName?: string) => {
    const matches = serverItems.filter(i => matchesProduct(i, id))
  ...
  const toggleWishlist = useCallback((id: number, productName?: string, variantId?: number | null, color?: string, size?: string) => {
    if (variantId !== undefined) {
      ...
      const wasLiked = serverItems.some(i => matchesExact(i, id, variantId))
      if (wasLiked) {
    ...
    const wasLiked = serverItems.some(i => matchesProduct(i, id))
    if (wasLiked) {
