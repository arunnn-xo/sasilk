...
export function validateImageFile(
  ...
  if (!dimensionHint || file.type === 'image/svg+xml') {
    return Promise.resolve({ valid: true as const })
  }
  const url = URL.createObjectURL(file)
  return imageDimensionsFromSrc(url)
    .then(({ width, height }) => {
      URL.revokeObjectURL(url)
      switch (dimensionHint) {
        case 'banner-hero':
          if (width < 1200 || height < 450) {
            return { valid: false as const, reason: `Image is too small (${width}×${height}px). Minimum: 1200×450px.` }
          }
          return { valid: true as const }
        case 'banner-other':
          if (width < 800 || height < 300) {
            return { valid: false as const, reason: `Image is too small (${width}×${height}px). Minimum: 800×300px.` }
          }
          return { valid: true as const }
        case 'product-card':
          if (width < 600 || height < 750) {
            return { valid: false as const, reason: `Image is too small (${width}×${height}px). Minimum: 600×750px.` }
          }
          return { valid: true as const }
        case 'category-card':
          if (width < 600 || height < 800) {
            return { valid: false as const, reason: `Image is too small (${width}×${height}px). Minimum: 600×800px.` }
          }
          return { valid: true as const }
        default:
          return { valid: true as const }
