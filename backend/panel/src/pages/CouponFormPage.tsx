  ...
  const validate = () => {
    ...
    if (type === 'percentage') {
      if (maxDiscount) {
        ...
        errors.maxDiscount = 'Please enter a max discount cap.'
      }
    }
    ...
    if (usageLimit) {
      ...
      errors.usageLimit = 'Please enter a total usage limit.'
    }

    const perUserNum = parseInt(perUserLimit, 10)
    if (!perUserLimit || isNaN(perUserNum) || perUserNum < 1) errors.perUserLimit = 'Per user limit must be at least 1.'

    if (minCartValue) {
      const mcv = parseFloat(minCartValue)
      if (isNaN(mcv) || mcv < 0) errors.minCartValue = 'Min cart value must be a positive number.'
    } else {
      errors.minCartValue = 'Please enter a minimum cart value.'
    }

    if (!startsAt) {
      errors.startsAt = 'Please select a start date.'
    }
    if (!expiresAt) {
        ...
        <form onSubmit={submit} className="p-6 md:p-8">
          <div className="grid gap-6 md:grid-cols-2">
            ...
            {type === 'percentage' && (
              <div className="space-y-2">
                <label className="block text-[14px] font-bold uppercase tracking-widest text-[var(--muted)]">
                  Max Discount Cap (₹) <span className="text-red-400">*</span>
                </label>
                <input
                  type="number"
                  min="0"
            ...
            <div className="space-y-2">
              <label className="block text-[14px] font-bold uppercase tracking-widest text-[var(--muted)]">
                Min Cart Value (₹) <span className="text-red-400">*</span>
              </label>
              <input
                type="number"
                min="0"
            ...
            <div className="space-y-2">
              <label className="block text-[14px] font-bold uppercase tracking-widest text-[var(--muted)]">
                Total Usage Limit <span className="text-red-400">*</span>
              </label>
              <input
                type="number"
                min="0"
