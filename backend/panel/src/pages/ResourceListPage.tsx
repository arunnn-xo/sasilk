...
export default function ResourceListPage({ config }: { config: ResourceConfig }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="bg-white rounded-2xl p-6 shadow-xl shadow-slate-200/50 border border-slate-100">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
          <div>
            ...
            <h1 className="mt-1 flex items-center gap-3 font-black text-2xl text-slate-800 tracking-tight md:text-3xl">
              <config.Icon className="h-6 w-6 text-blue-600 md:h-7 md:w-7" />
              {config.title}
            </h1>
          </div>
      ...
      {/* ─── Data Table ───────────────────────────────────── */}
      <section className="bg-white rounded-2xl p-6 shadow-xl shadow-slate-200/50 border border-slate-100 space-y-4">
        {/* Record count bar */}
        {!isLoading && items.length > 0 ? (
          <div className="pb-2 flex justify-between items-center border-b border-slate-100">
            ...
            <tbody className="divide-y divide-slate-100">
              {items.map((item: any, idx: number) => (
                ...
                >
                  ...
                  {!config.hideSerialNumber && (
                    <td className="px-5 py-4 font-bold text-slate-400 text-xs">{(currentPage - 1) * ITEMS_PER_PAGE + idx + 1}</td>
                  )}
                  {visibleColumns.map(column => (
                    ...
                    <div className="flex flex-col items-center gap-3">
                      ...
                      {!config.hideAddNew && (
                        <button
                          type="button"
                          onClick={() => navigate(`/${basePath}/new`)}
                          className="mt-1 inline-flex items-center gap-2 bg-[#F59E0B] hover:bg-[#D97706] text-slate-950 font-bold px-4 py-2 rounded-full text-xs transition"
                        >
