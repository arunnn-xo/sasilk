...
export const categoryCreateSchema = z.object({
  ...
  name: z.string().min(3, 'Category name must be at least 3 characters.').max(30, 'Category name cannot exceed 30 characters.'),
...
export const resourceConfig: Record<string, ResourceConfig> = {
  ...
  categories: {
    ...
    beforeSave: body => ({
      ...body,
      slug: body.slug || slugify(`${body.section || ''}-${body.name || ''}`),
    }),
    useForceDelete: true,
  ...
  coupons: {
    ...
    validationSchema: z.object({
      ...
      minCartValue: z.number().min(0).optional().default(0),
      maxDiscount: z.union([z.number().min(0), z.null()]).optional(),
  ...
  orders: {
    model: Order,
    entity: 'order',
    writable: ['status', 'paymentStatus', 'deliveryAgentName', 'deliveryAgentPhone', 'trackingNumber', 'dispatchedAt', 'deliveredAt', 'metadata'],
    defaultOrder: [['id', 'DESC']],
