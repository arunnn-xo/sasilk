...
router.post('/login', asyncHandler(async (req, res) => {
  ...
  if (!admin) throw new AppError(401, 'Invalid admin email or password.')
  ...
  const token = signAccessToken({ sub: safeAdmin.id, email: safeAdmin.email, role: safeAdmin.role, type: 'admin' })
