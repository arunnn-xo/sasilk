...
export async function runMigrations() {
  ...
  await createTableIfMissing(qi, 'refunds', {
    ...
    initiated_by: { type: DataTypes.INTEGER.UNSIGNED, allowNull: true },
  ...
  await safeAddColumn('products', 'hsn_code', { type: DataTypes.STRING(20), allowNull: true, defaultValue: '5804' })
  await safeAddColumn('products', 'weight_kg', { type: DataTypes.DECIMAL(8, 3), allowNull: true })
  await safeAddColumn('products', 'length_cm', { type: DataTypes.DECIMAL(8, 2), allowNull: true })
  await safeAddColumn('products', 'breadth_cm', { type: DataTypes.DECIMAL(8, 2), allowNull: true })
  await safeAddColumn('products', 'height_cm', { type: DataTypes.DECIMAL(8, 2), allowNull: true })
  ...
  await createTableIfMissing(qi, 'stock_notifications', {
    id: { type: DataTypes.INTEGER.UNSIGNED, autoIncrement: true, primaryKey: true },
    product_id: { type: DataTypes.INTEGER.UNSIGNED, allowNull: false, references: { model: 'products', key: 'id' }, onDelete: 'CASCADE' },
