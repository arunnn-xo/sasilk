...
export function generateAddressesPdf(orders: Array<Record<string, unknown>>): PDFKit.PDFDocument {
  const doc = new PDFDocument({
    size: 'A4',
    margins: { top: 36, bottom: 36, left: 40, right: 40 },
    info: {
      Title: 'Confirmed Orders - Customer Addresses',
      Author: 'Threads of TN',
    },
  })
  ...
  doc.text('Confirmed Orders - Customer Addresses', leftCol, y, { width: pageWidth, align: 'center' })
  y += 28
  ...
  for (let i = 0; i < orders.length; i++) {
    ...
    if (shippingAddress) {
      ...
      if (shippingAddress.phone) {
        doc.text(`Phone: ${String(shippingAddress.phone)}`, leftCol, y)
        y += 18
      }
    } else {
    ...
    if (order.customerEmail) {
      doc.text(`Email: ${String(order.customerEmail)}`, leftCol, y)
      y += 18
    }
