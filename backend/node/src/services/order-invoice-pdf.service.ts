...
function registerUnicodeFont(doc: PDFKit.PDFDocument) {
...
export function generateInvoicePdf(order: Record<string, unknown>, invoice: Record<string, unknown>, company?: CompanyInfo): PDFKit.PDFDocument {
  ...
  const { fontRegular, fontBold } = registerUnicodeFont(doc)
  ...
  function resolveLogoPath(): string | null {
    ...
    candidates.push(path.join(cwd, 'uploads', 'threads-of-tn-logo.png'))
    // Fallback 2: frontend public folder (works in dev when running from backend/node)
    candidates.push(path.join(cwd, '..', '..', 'frontend', 'public', 'threads-of-tn-logo-transparent.png'))
    candidates.push(path.join(cwd, '..', 'frontend', 'public', 'threads-of-tn-logo-transparent.png'))
  ...
  if (customer) {
    const nameStr = [customer.name, customer.company].filter(Boolean).join(', ')
    doc.text(nameStr || 'Customer', leftCol, y)
    doc.text(`Email: ${String(customer.email || order.customerEmail || '')}`, leftCol, y + 12)
    doc.text(`Phone: ${String(customer.mobile || shippingAddress?.phone || '')}`, leftCol, y + 24)
    if (customer.gstin) {
      doc.text(`GSTIN: ${String(customer.gstin)}`, leftCol, y + 36)
    }
  } else {
    const guestName = [shippingAddress?.firstName, shippingAddress?.lastName].filter(Boolean).join(' ')
    doc.text(guestName || 'Guest', leftCol, y)
    doc.text(`Email: ${String(order.customerEmail || 'Guest')}`, leftCol, y + 12)
    if (shippingAddress?.phone) {
      doc.text(`Phone: ${String(shippingAddress.phone)}`, leftCol, y + 24)
    }
  }
  ...
  if (shippingAddress) {
    const addrLines = [
      ...
      [String(shippingAddress.city || ''), String(shippingAddress.state || '')].filter(Boolean).join(', '),
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    doc.text(String(item.name || ''), colX.product, y, { width: 120 })
  ...
  y = Math.max(y, headerY + 26 + items.length * 18) + 8
  doc.moveTo(leftCol, y).lineTo(leftCol + pageWidth, y).strokeColor('#ddd').stroke()
  y += 8
