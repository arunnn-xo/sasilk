...
function resolveLogoAttachment(company: any): any | null {
  const localLogo = resolveLogoPath()
  if (localLogo) {
    return {
      filename: 'logo.png',
      path: localLogo,
...
export async function sendInvoiceEmail(
  ...
  const html = wrapInEmailTemplate(
    ...
    <div style="margin-top: 24px; text-align: center;">
      <a href="${frontendUrl}/account/orders" style="display: inline-block; background: linear-gradient(135deg, #4A0F1C, #6B1A2A); color: #ffffff; padding: 12px 32px; border-radius: 8px; text-decoration: none; font-size: 14px; font-weight: 700; letter-spacing: 1px;">Track Your Order →</a>
    </div>
  ...
  try {
    await transporter.sendMail({
      ...
      text: [
        ...
        ``,
        `Track your order at: ${frontendUrl}/account/orders`,
        ``,
        `Regards,`,
        `${company.name}`,
      ].join('\n'),
...
export async function sendPriceDropEmail(
  ...
  let logoAttachment: Record<string, unknown> | null = null
  if (company.logoUrl) {
    const logoPath = path.resolve(company.logoUrl.replace(/^\//, ''))
    if (fs.existsSync(logoPath)) {
      logoAttachment = { cid: 'logo', filename: path.basename(logoPath), path: logoPath }
    }
  }
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="background-color:#FBF9F6;padding:24px 40px;text-align:center;border-bottom:3px solid #6B1A2A;">
              ${logoAttachment ? `<img src="cid:logo" alt="${company.name}" style="max-width:140px;height:auto;display:inline-block;margin-bottom:8px;" />` : ''}
...
export async function sendGeneralEmail(
  ...
  let bannerAttachment: Record<string, unknown> | null = null
  let bannerCid = ''
  if (campaign.imageUrl) {
    const bannerPath = path.resolve(campaign.imageUrl.replace(/^\//, ''))
    if (fs.existsSync(bannerPath)) {
      bannerCid = `banner_${Date.now()}`
      bannerAttachment = { cid: bannerCid, filename: path.basename(bannerPath), path: bannerPath }
    }
  }

  const bannerHtml = bannerCid
    ? `<tr><td style="padding:0;"><img src="cid:${bannerCid}" alt="" style="width:100%;height:auto;display:block;max-width:600px;" /></td></tr>`
    : ''
...
export async function sendPackingEmail(
  ...
  const html = wrapInEmailTemplate(
    ...
    <div style="margin-top: 24px; text-align: center;">
      <a href="${frontendUrl}/account/orders" style="display: inline-block; background: linear-gradient(135deg, #4A0F1C, #6B1A2A); color: #ffffff; padding: 14px 40px; border-radius: 8px; text-decoration: none; font-size: 15px; font-weight: 700; letter-spacing: 1px;">View My Order →</a>
    </div>
...
export async function sendShippingEmail(
  ...
  const courierName = String(meta.shiprocketCourierName || order.deliveryAgentName || 'Our Courier Partner')
  const trackingNumber = String(order.trackingNumber || meta.shiprocketAwbCode || '')
  const trackingUrl = trackingNumber
    ? `https://shiprocket.co/track/${trackingNumber}`
    : ''
  ...
  const html = wrapInEmailTemplate(
    ...
    <div style="margin-top: 24px; text-align: center;">
      <a href="${frontendUrl}/account/orders" style="display: inline-block; background: linear-gradient(135deg, #4A0F1C, #6B1A2A); color: #ffffff; padding: 14px 40px; border-radius: 8px; text-decoration: none; font-size: 15px; font-weight: 700; letter-spacing: 1px;">View Order →</a>
    </div>`}
...
export async function sendDeliveryEmail(
  ...
  const orderNumber = String(order.orderNumber || '')
  const frontendUrl = env.FRONTEND_URL || 'http://localhost:3000'
  const reviewUrl = `${frontendUrl}/account/orders`
