...
export function checkRatioMatch(rule: DimensionRule, width: number, height: number): boolean {
  if (!rule.recommendedRatio) return true
  const [rw, rh] = rule.recommendedRatio.split(':').map(Number)
...
export async function validateImageDimensions(filePath: string, rule: DimensionRule): Promise<ValidationResult> {
  if (!sharp) {
  ...
  if (width < rule.minWidth || height < rule.minHeight) {
    return {
      valid: false,
      reason: `Image is too small (${width}×${height}px). ` +
        `${rule.description} Minimum: ${rule.minWidth}×${rule.minHeight}px.`,
      dimensions: { width, height },
    }
  }

  if (!checkRatioMatch(rule, width, height)) {
    return {
      valid: false,
      reason: `Image aspect ratio (${width}×${height}px) doesn't match the recommended ${rule.recommendedRatio} ratio. ` +
        `Your image has a ${(width / height).toFixed(2)}:1 ratio. Please crop or resize to ${rule.recommendedRatio}.`,
      dimensions: { width, height },
    }
  }
